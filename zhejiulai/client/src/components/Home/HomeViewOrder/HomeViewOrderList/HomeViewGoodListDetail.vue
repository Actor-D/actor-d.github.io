...
const fetchOrderDetail = async () => {
  try {
    ...
    const response = await axios.get(`http://127.0.0.1:5000/api/good/${order.value.id}`, {
    ...
    order.value = {
      ...
      orderTime: new Date(new Date(data.created_at).getTime() + 8 * 60 * 60 * 1000).toLocaleString(),
      items: [{
        id: 1,
        name: data.good_name,
        size: data.good_size,
      ...
      deliveryFee: data.paymentMethod === 'points'
        ? data.delivery_fee_points
        : data.delivery_fee_cash,
      total: data.paymentMethod === 'points'
        ? `${data.delivery_fee_points}积分`
        : `¥${data.delivery_fee_cash}`,
      paymentMethod: data.paymentMethod === 'points' ? '积分支付' : '现金支付'
    };
...
//       `http://127.0.0.1:5000/api/good/${order.value.id}/images`,
//       {
//         headers: { 'Authorization': `Bearer ${token}` }
//       }
//     );
...
<template>
  <div class="order-detail-container" :class="{ 'animated': animated }">
        ...
        <div class="item-list">
          <div v-for="item in order.items" :key="item.id" class="item">
            ...
            <div class="item-info">
              ...
              <div class="info-row">
                <!-- <span class="info-label">订单大小</span> -->
                <span class="info-value">订单大小：{{ item.size }}</span>
              </div>
    ...
    <div class="button-content">
      ...
      <button class="btn-reorder"><router-link to="/Home/CreateOrder/Good" class="nav-link">再来一单</router-link></button>
    </div>
  </div>
</template>
