...
const fetchOrder = async () => {
  try {
    ...
    const paymentMethod = response.data.paymentMethod === 'points' ? '积分支付' :
                      response.data.paymentMethod === 'cash' ? '现金支付' :
                      '未知支付方式';
    const deliveryFees = response.data.paymentMethod === 'points'
      ? `${response.data.delivery_fee_points}积分`
      : `¥${response.data.delivery_fee_cash}`

    orderInfo.value = {
      ...orderInfo.value,
      orderId: response.data.goodorder_id,
      good_name: response.data.good_name,
      receiver_address: response.data.receiver_address,
      paymentMethod,
      deliveryFees,
