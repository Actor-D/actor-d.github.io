...
const userInfo = ref({ name: '', student_id: '', phone: '', points: 0 });
const orderInfo = ref({
  good_name: '',
  good_size: '',
  good_type: '',
...
const paymentMethods = ref([
  ...
  { id: 3, name: '积分支付（金额计算未实现[因为外卖部分已经实现了，这里不做展示]）', checked: false, type: 'points' }
]);
...
const fetchOrder = async () => {
  try {
    ...
    const response = await axios.get(`http://127.0.0.1:5000/api/good/${orderId}`, {
      headers: { 'Authorization': `Bearer ${token}` }
    });
...
const submitPayment = async () => {
  ...
  try {
    ...
    const response = await axios.put(
      `http://127.0.0.1:5000/api/good/${orderId}/payment`,
    ...
    if (response.data.success) {
      router.push('/Home/CreateOrder/GoodPay');
    }
  } catch (error) {
...
<template>
  ...
  <!-- 支付方式选择错误提示 -->
  <div v-if="showPaymentError" class="payment-error-alert">
    <div class="alert-content">
      <span class="alert-icon">⚠️</span>
      请选择支付方式
    </div>
  ...
  <div class="order-container">
    ...
    <div class="order-content height1">
      <div class="information">
        ...
        <div class="input-group">
          <div class="order-label">{{ orderInfo.receiver_address }}</div>
    ...
    <button class="button-content back-button">
      <router-link to="/Home/CreateOrder/GoodAddress" class="nav-link">上一步</router-link>
    </button>
  </div>
</template>
