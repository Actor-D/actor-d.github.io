const translations = {
  zh: {
    ...
    projects: {
      ...
      previewSummary:
        "连接用户认证、多步骤下单、地址管理、支付选择与订单查询的完整前后端业务流程——以学生身份为起点，把校园配送的每一个环节数字化。",
      viewFull: "查看完整案例",
      highlightOne: "学号注册登录、密码加密与受保护路由",
      highlightTwo: "订单创建、地址选择、加急费用与多种支付方式",
      highlightThree: "订单状态管理、图片上传与用户级 API 权限",
  ...
  en: {
    ...
    projects: {
      ...
      previewSummary:
        "A complete front-to-back workflow connecting authentication, multi-step ordering, address management, payment selection, and order tracking — starting from student identity to digitize every step of campus delivery.",
      viewFull: "View the full case study",
      highlightOne: "Student sign-up/login, password hashing, and protected routes",
      highlightTwo: "Order creation, address selection, priority pricing, and payment options",
      highlightThree: "Order management, image uploads, and user-scoped API access",
